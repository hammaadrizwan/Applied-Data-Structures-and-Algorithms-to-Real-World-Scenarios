...
public class Main {
    ...
    private static String getCode(String word){
        ...
        ArrayList<Character> cleaningTheWord = new ArrayList<>();
        for (char character:characters) {
            if( character!=' '){
                cleaningTheWord.add(character);
            }
        }
        char[] lettersCleaned = new char[cleaningTheWord.size()];
        for (int i = 0; i < cleaningTheWord.size(); i++) {
            lettersCleaned[i] = cleaningTheWord.get(i);
        }
        String cleanedWord = new String(lettersCleaned);
        char[] encodedCharacters = cleanedWord.toCharArray();

        ArrayList<Character> listOfLetters = new ArrayList<>();
        code[0]=encodedCharacters[0];
        for (int i = 1; i < 4; i++) {
            if (i>=encodedCharacters.length){
                code[i]='0';
            }else{
                code[i]=encodedCharacters[i];
            }
    ...
    public static ArrayList<String> phoneticCode(BinaryTree binaryTree, String data) {
        ArrayList<String> possibleWords = new ArrayList<>();
        String codeOfTarget = getCode(data);
        if (binaryTree.searchCode(codeOfTarget) != (null)) {//checks if that is a word, does this untill all the characters are being entered to all the positions of the word
            String possibleWord= binaryTree.searchCode(codeOfTarget);
            possibleWords.add(possibleWord);
        }
        //Find the code for all the characters that have a simillar code
...
//        }
    ...
    public static BinaryTree readFromFile(String filename) throws FileNotFoundException {
        ...
        while (readFile.hasNextLine()) {//until end of file the data is being added to the tree.
            ...
            binaryTree.insert(data.trim(),code);
        }
        readFile.close();
        return binaryTree;
    }
